...
def customize_process_and_associate(process, isMC, disTauTagOutputOpt = 1) :
    ...
    process.disMuonTable = simpleCandidateFlatTableProducer.clone(
        ...
        variables = cms.PSet(CandVars,
            ...
            mediumId = Var("globalTrack().isNonnull()?(isGlobalMuon) && (globalTrack().normalizedChi2() < 3) && (combinedQuality().chi2LocalPosition < 12) && (combinedQuality().trkKink < 20) && (innerTrack().validFraction() > 0.8)?(segmentCompatibility() > 0.303):(segmentCompatibility() > 0.451):0",bool,doc="cut-based ID, medium WP"),
            mediumPromptId = Var("passed('CutBasedIdMediumPrompt')",bool,doc="cut-based ID, medium prompt WP"),
              tightId = Var("?globalTrack().isNonnull() && innerTrack.isNonnull()?(globalTrack().normalizedChi2() < 10) && (numberOfMatchedStations() > 1) && (globalTrack().hitPattern().numberOfValidMuonHits() > 0) && (innerTrack().hitPattern().numberOfValidPixelHits() > 0) && (innerTrack().hitPattern().trackerLayersWithMeasurement() > 5):0",bool,doc="cut-based ID, tight WP"),
